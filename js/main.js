...
$(document).ready(function () {
  ...
  SQUARES.forEach((square, index) => {
    $(square).click(function () {
      playMove([Math.floor(index / 3), index % 3]);
    });
  });
  ...
  function isDraw() {
    for (let i = 0; i < MAX_COLUMN; i++)
      for (let j = 0; j < MAX_ROW; j++) if (board[i][j] == EMPTY) return false;
    return true;
  }
  ...
  function updateBoard(move) {
    ...
    if (squareNum < 0 || squareNum > 9) throw new Error("Invalid Move.");
  ...
  function playMove(move) {
    if (board[move[0]][move[1]] == EMPTY) {
      ...
      if (isWon()) {
        $("#info").html(
          $("#level-select").val() == "Bot Off"
            ? `${currentPlayer} Won!`
            : INFO_MESSAGES.userWon,
        );
      } else if (isDraw()) {
        $("#info").html(INFO_MESSAGES.draw);
      } else if (level != 0) {
