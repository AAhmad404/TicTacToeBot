    ...
    */
    #evaluateGame(board) {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                for (let j = 0; j < MAX_ROW; j++) {
                    if (board[0][j] == players[i] 
                        && board[1][j] == players[i]
                        && board[2][j] == players[i])
                        return players[i] == X? 1 : -1;
                }
                if (board[0][0] == players[i] && board[1][1] == players[i] && board[2][2] == players[i]) {
                    return players[i] == X? 1 : -1;
                }
                if (board[0][2] == players[i] && board[1][1] == players[i] && board[2][0] == players[i]) {
                    return players[i] == X? 1 : -1;
                }
            }
    ...
    #getLevelThreeMove(board, currentPlayer) {
        ...
        if (currentPlayer == X) {
            validMoves.forEach((move) => {
                ...
                let evalutation = this.#minimax(board, Infinity, O, -Infinity, Infinity);
                if (evalutation > bestEval) {
                    moves = [];
                    moves.push(move);
                    bestEval = evalutation;
            ...
            validMoves.forEach((move) => {
                this.#makeMove(board, move, currentPlayer);
                let evalutation = this.#minimax(board, Infinity, X, -Infinity, Infinity);
                if (evalutation < bestEval) {
                    moves = [];
                    moves.push(move);
                    bestEval = evalutation;
